...
The goal is to explain the project clearly without making it sound bigger than it is. The app is a polished MVP that demonstrates React, form handling, state updates, local persistence, a small Supabase insert proof, testing, deployment, and responsible product boundaries.

## One-Minute Explanation
SoundCare AI Booking CRM is a React + Vite portfolio MVP for a fake hearing-care clinic booking workflow. A visitor can submit a fake booking request, the app saves it locally for the demo dashboard, analytics update from the same booking data, and a mock assistant can generate safe administrative draft text. The deployed version also proves that fake booking submissions can be inserted into Supabase using browser-safe environment variables, while dashboard reads and status updates stay local until an authenticated admin phase exists.

## Three-Minute Demo Flow
1. Open the live app: `https://soundcare-ai-booking-crm.vercel.app/`.
2. Explain that the project uses fake/demo booking data only.
3. Submit a fake booking request through the public form.
4. Show that the analytics and dashboard update in the app.
...
7. Generate a mock assistant draft and point out the non-medical administrative safety line.
8. Mention the test suite, Vercel deployment, and docs as evidence of project quality.
...
- I kept the AI assistant mock/template-based because real AI would introduce privacy, safety, cost, and API-key concerns that are not needed for this junior portfolio MVP.
...
### Why is the AI assistant not real AI?
